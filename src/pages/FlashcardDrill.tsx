...
export default function FlashcardDrill() {
  ...
  // Update waktu real-time tiap ganti card
  useEffect(() => {
    setCurrentTime(Date.now());
  }, [currentIndex]);

  // Reset flip pas ganti kategori (INI YANG BIKIN ERROR TADI)
  useEffect(() => {
    setIsFlipped(false);
  }, [activeCategory]);
  ...
  const getReviewText = () => {
    ...
    if (cardProgress.nextReview < currentTime) return "Due";
    const daysLeft = Math.ceil((cardProgress.nextReview - currentTime) / (1000 * 60 * 60 * 24));
    return `${daysLeft}h`;
  };
  ...
  return (
    <div className="p-4 md:p-6 pb-24 animate-in fade-in max-w-3xl mx-auto font-sans">
      ...
      {filteredCards.length > 0 ? (
            ...
            >
              <div className="absolute top-4 left-4 flex gap-2">
                ...
                <span
                  className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full flex items-center gap-1 ${
                    !cardProgress || cardProgress.nextReview < currentTime
              ...
              <div className="absolute top-4 right-4">
                {(currentCard as any).type === 'premium' ? (
                  ...
                  <span className="bg-teal-100 text-teal-600 text-[9px] font-bold px-2 py-0.5 rounded border border-teal-200">FREE</span>
                )}
              </div>
              ...
              <div className="absolute bottom-6 text-teal-500 text-xs font-bold flex items-center gap-2 bg-teal-50 dark:bg-teal-900/30 px-3 py-1.5 rounded-full">
                <RotateCw size={12} /> Lihat Jawaban
              </div>
            </div>
        ...
        <div className="h-80 flex flex-col items-center justify-center text-slate-400 bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700">
          <Zap size={32} className="mb-2 opacity-50" />
          <p className="text-sm font-medium">Kategori ini selesai!</p>
          <button
            onClick={() => { localStorage.removeItem('medprep_flashcard_progress'); window.location.reload(); }}
      ...
      {!isFlipped && filteredCards.length > 0 && (
        <div className="flex items-center justify-center gap-4 mt-6 opacity-60 hover:opacity-100 transition-opacity">
          <button onClick={handlePrev} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400">
            <ChevronLeft size={20} />
          </button>
          <span className="text-xs text-slate-400 font-mono font-bold">
            {currentIndex + 1} / {filteredCards.length}
          </span>
          <button onClick={handleNext} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400">
